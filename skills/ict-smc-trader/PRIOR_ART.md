...
| `pine/boom-hunter-pro.pine` | veryfid's indicator, CC BY-NC-SA. Source obtained; live extractor (`signal-grid.js`) updated; **no signal-bus reimplementation exists** (no `scripts/signal-bus/boom-hunter/`). | engineering-complete (live extraction only) / **missing** (deeper zone/touch/significance analysis) | Live-readable fields only: `Long gray/yellow/blue/Lime`, `quotient_1/quotient_2`, `momentum_direction`. Everything else flagged unknown | That Boom Hunter has been backtested or significance-tested at all — it has not, at any level beyond live extraction |
