#!/usr/bin/env node
// Cost/capacity test on the cross-indicator SuperTrend confluence significance test (companion to
// cipher-a-supertrend-confluence-significance.js). Two-stage bar as everywhere else in this
// register: a real significance result still has to clear real costs before counting as validated.
//
// Usage: node scripts/signal-bus/cross-confluence/cipher-a-supertrend-confluence-cost-capacity-backtest.js --signal-type=bull_candle [--r=1,1.5,2,3] [--fee-tier=bitunix_futures_vip1]
...
async function buildTrades(rMultiple, candlesByTf, atrByTf) {
  ...
  for (const tf of LADDER_KEYS) {
    ...
    for (const ev of events) {
      ...
      trades.push({ side, entryTime, entryPrice, exitTime: result.exitTime, exitPrice: result.exitPrice, pnlPct, confluence, timeframe: tf });
    }
  }
  return trades;
}
...
async function main() {
  ...
  console.log(`Signal: ${SIGNAL_TYPE} x SuperTrend confluence. Fee tier: ${FEE_TIER} (taker=${(FEE_TIERS[FEE_TIER].takerFeePct * 100).toFixed(3)}%, round-trip=${(FEE_TIERS[FEE_TIER].takerFeePct * 200).toFixed(3)}%)`);
  const allResults = {};
  ...
  for (const rMult of R_MULTIPLES) {
    ...
    const confluence = trades.filter((t) => t.confluence), noConfluence = trades.filter((t) => !t.confluence);
    console.log(`-- confluence vs no-confluence (pooled) --`);
    const r = { confluence: reportBucket("confluence", confluence, confirmedParams), noConfluence: reportBucket("no-confluence", noConfluence, confirmedParams) };
    console.log(`-- confluence, per timeframe --`);
    const byTf = {};
    for (const tf of LADDER_KEYS) {
      const tfConf = confluence.filter((t) => t.timeframe === tf);
      byTf[tf] = reportBucket(tf, tfConf, confirmedParams);
    }
    allResults[`${rMult}R`] = { tradeCount: trades.length, confluenceVsNo: r, byTf };
  }

  const RESULTS_DIR = new URL("results/", import.meta.url);
  mkdirSync(RESULTS_DIR, { recursive: true });
  const payload = { signalType: SIGNAL_TYPE, results: allResults, git_commit: gitCommit(), generated_at: new Date().toISOString() };
  const fname = `cipher_a_${SIGNAL_TYPE}_supertrend_confluence_cost_capacity_backtest_${payload.generated_at.replace(/[:.]/g, "-")}.json`;
