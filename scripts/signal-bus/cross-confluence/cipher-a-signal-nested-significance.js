#!/usr/bin/env node
// Generalized version of cipher-a-bullcandle-nested-significance.js (#71) -- extends the same
// nested cross-timeframe confirmation test to Cipher A's other 6 signals (yellowCross, greenDot,
// redCross, blueTriangle, redDiamond, bloodDiamond), flagged as the natural next step in the
// original build-out plan. Side-aware (long on bullish events, short on bearish) since most of
// these signals are single-sided (yellowCross/redCross/redDiamond/bloodDiamond = bearish only,
// blueTriangle = bullish only) but greenDot has both -- nesting requires a SAME-SIDE slower-TF
// event, matching the convention used throughout this whole nested-confirmation line of work.
//
// Same corrected sequential methodology (slower TF must precede, TF-scaled window), 0.6xATR(14)
// fixed-R (no natural OB/stop for these signals either).
//
// Usage: node scripts/signal-bus/cross-confluence/cipher-a-signal-nested-significance.js --signal-type=yellow_cross [--iterations=20000] [--r=1,1.5] [--price-tolerance=0.01] [--nested-window=10]
...
if (!SIGNAL_TYPE) { console.error("Fatal: --signal-type is required (yellow_cross|green_dot|red_cross|blue_triangle|red_diamond|blood_diamond|bull_candle)"); process.exit(1); }
...
// Same-side confirmation only -- a bullish event is confirmed by a slower TF's own bullish event
// of the SAME type, not just "any event," matching every other nested test in this project.
...
async function buildOutcomes(rMultiple, candlesByTf, atrByTf) {
  ...
  for (const tf of LADDER_KEYS) {
    ...
    for (const ev of events) {
      ...
      results.push({ nested: nestedOn.length > 0, depth: nestedOn.length, timeframe: tf, side: ev.side, wins: [outcome] });
    }
  }
  return results;
}
...
async function runForRMultiple(rMultiple, candlesByTf, atrByTf, iterations, seed) {
  ...
  console.log(`${obs.length} ${SIGNAL_TYPE} events (nested=${nNested}, solo=${nSolo}), ${totalTrades} resolved trades.`);
  ...
  return { rMultiple, obCount: obs.length, tradeCount: totalTrades, nNested, nSolo, test, byTf };
}
...
async function main() {
  ...
  const payload = { signalType: SIGNAL_TYPE, results, priceTolerance: PRICE_TOLERANCE_PCT, nestedWindowBars: NESTED_WINDOW_BARS, git_commit: gitCommit(), generated_at: new Date().toISOString() };
  const fname = `cipher_a_${SIGNAL_TYPE}_nested_significance_${payload.generated_at.replace(/[:.]/g, "-")}.json`;
