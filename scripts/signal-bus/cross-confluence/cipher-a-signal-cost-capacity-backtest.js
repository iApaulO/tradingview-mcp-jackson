#!/usr/bin/env node
// Cost/capacity test on #77 (Cipher A signal nested confirmation, generalized). Same two-stage
// bar as every other claimed-real finding in this register -- red_cross and green_dot tested real
// and significant in #77, checked here against real costs before either counts as validated.
// Side-aware (long/short), same 0.6xATR(14) fixed-R construction as #71/#77's significance test.
//
// Usage: node scripts/signal-bus/cross-confluence/cipher-a-signal-cost-capacity-backtest.js --signal-type=red_cross [--r=1,1.5,2,3] [--fee-tier=bitunix_futures_vip1]
...
const ATR_MULT = args["atr-mult"] ? Number(args["atr-mult"]) : 0.6;
...
async function main() {
  ...
  console.log(`Signal: ${SIGNAL_TYPE}. ATR mult: ${ATR_MULT}. Fee tier: ${FEE_TIER} (taker=${(FEE_TIERS[FEE_TIER].takerFeePct * 100).toFixed(3)}%, round-trip=${(FEE_TIERS[FEE_TIER].takerFeePct * 200).toFixed(3)}%)`);
  const allResults = {};
  ...
  for (const rMult of R_MULTIPLES) {
    ...
    allResults[`${rMult}R`] = { tradeCount: trades.length, nestedVsSolo: r };
  }

  const RESULTS_DIR = new URL("results/", import.meta.url);
  mkdirSync(RESULTS_DIR, { recursive: true });
  const payload = { signalType: SIGNAL_TYPE, atrMult: ATR_MULT, feeTier: FEE_TIER, results: allResults, git_commit: gitCommit(), generated_at: new Date().toISOString() };
  const fname = `cipher_a_${SIGNAL_TYPE}_atrmult${ATR_MULT}_cost_capacity_backtest_${payload.generated_at.replace(/[:.]/g, "-")}.json`;
