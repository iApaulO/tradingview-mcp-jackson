#!/usr/bin/env node
// Permutation significance test for SMC's confluence-vs-hold-rate finding, same method as
// divergence-for-many/confluence-significance.js: shuffle confluence_count labels across ORDER
// BLOCKS (the entity the label belongs to), keep each block's own real touch outcomes, recompute
// the statistic, repeat. Touch-level shuffling would pseudo-replicate (a block's touches aren't
// independent observations of it) and understate the null's true variance.
//
// The observed gradient here is much larger than Divergence for Many's (34.7% at confluence=1 to
// 68.0% at confluence=8, vs. 53.4% to 60.6% there) -- a bigger effect size is MORE reason to test
// carefully, not less, given the 82-88% hold-rate bug earlier this session looked dramatic too and
// turned out to be wrong.
//
// Usage: node scripts/signal-bus/smc/confluence-significance.js --iterations=50000
...
function topVsBottomGap(xs, ys) {
  ...
  return heldTop / nTop - heldBottom / nBottom;
}

export function runSMCConfluenceSignificanceTest({ iterations = 5000, seed = 42, dbPath = DB_PATH } = {}) {
  const db = new DatabaseSync(dbPath, { readOnly: true });
  const rows = db.prepare(
    `SELECT ob.id as ob_id, ob.confluence_count, obt.outcome
  ...
  const obMap = new Map();
  for (const r of rows) {
    if (!obMap.has(r.ob_id)) obMap.set(r.ob_id, { confluenceCount: r.confluence_count, outcomes: [] });
    obMap.get(r.ob_id).outcomes.push(r.outcome === "held" ? 1 : 0);
  }
  const obs = [...obMap.values()];

  const realLabels = obs.map((o) => o.confluenceCount);
  const realX = [], realY = [];
  for (const o of obs) for (const y of o.outcomes) { realX.push(o.confluenceCount); realY.push(y); }

  const realR = pointBiserial(realX, realY);
  const realGap = topVsBottomGap(realX, realY);
...
function main() {
  const result = runSMCConfluenceSignificanceTest({ iterations: ITERATIONS, seed: SEED });
  ...
  console.log(`Real top(8)-vs-bottom(1) gap: ${(result.gap.real * 100).toFixed(2)} points`);
  ...
  const { p: pR } = result.correlation, { p: pGap } = result.gap;
  console.log(`\nVerdict: ${pR < 0.05 && pGap < 0.05 ? "Both statistics clear 5% -- real, not a labeling artifact." : pR < 0.05 || pGap < 0.05 ? "Mixed -- unresolved." : "Neither clears 5% -- does NOT survive the test."}`);
}
