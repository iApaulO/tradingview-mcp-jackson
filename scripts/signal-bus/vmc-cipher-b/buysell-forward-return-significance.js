#!/usr/bin/env node
// Phase 1 of the video-driven implementation plan (2026-07-31): tests buySignal/sellSignal (the
// "green dot"/"red dot" -- WT1/WT2 cross at oversold/overbought, no divergence requirement) the
// same way forward-return-significance.js tested divergence: signed return over N bars from the
// signal bar, vs. a same-timeframe randomly-sampled-bar-and-side baseline. Pooled across all 8
// timeframes first, then stratified -- §18.4 already showed pooling can hide a timeframe-specific
// effect, so both views are produced in one run rather than needing a second script.
//
// Usage: node scripts/signal-bus/vmc-cipher-b/buysell-forward-return-significance.js
